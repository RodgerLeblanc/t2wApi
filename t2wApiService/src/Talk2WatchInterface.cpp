...
{
	QSettings settings("Roger Leblanc", "t2wApi_HL");
	QString defaultValue = "Free";

	// Send message via UDP 9877 (Pro) or 8989 (Free)
	// UDP 8989 is prefered for T2W Free because it is the default value (as of 2014-04-06)
	if (settings.value("t2wVersion", defaultValue).toString() == "Pro")
		m_udp->sendMessage("127.0.0.1", 9877, _command);
	else if (settings.value("t2wVersion", defaultValue).toString() == "Free")
...
{
	// This is not used with headless apps as an HL app can't invoke another app
...
void Talk2WatchInterface::onTalk2WatchLookup()
{
	...
	QSettings settings("Roger Leblanc", "t2wApi_HL");
	QString defaultValue = "Free";
	QString t2wVersion = settings.value("t2wVersion", defaultValue).toString();

	// If T2W was found in onTalk2WatchLookup() from UI part, then emit signal
	if ((t2wVersion == "Pro") || (t2wVersion == "Free"))
		emit transmissionReady();
	else
		qDebug() << "T2W not found by UI part";

	/*
	 * This is not good for headless apps as the invocation method can't be used
	 *
